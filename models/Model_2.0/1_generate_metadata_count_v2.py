...
COLOR_MAP = {
    ...
    'DrawingRoom': [0, 128, 0],     # Green
    'Garage': [165, 42, 42],         # Brown
    'Lounge': [255, 255, 0], # Yellow (Corrected Name)
    'Backyard': [50, 205, 50],       # Lime Green
    'Stairs': [0, 128, 128],         # Teal
    'Store': [128, 0, 128], # Purple (Corrected Name)
    'OpenSpace': [0, 255, 255],     # Cyan
    'PrayerRoom': [127, 127, 127],  # Grayish/Check RGB - Assuming original was right
    'Staircase': [153, 51, 255],     # Violet
    'Lobby': [255, 0, 255],          # Magenta
    'Lawn': [64, 224, 208],          # Turquoise
    'Dining': [225, 192, 203],       # Pink
    'ServantQuarter': [75, 0, 130], # Indigo (Corrected Name)
    'Passage': [128, 128, 0],        # Olive Green
    'Laundry': [230, 230, 250],      # Lavender
    'DressingArea': [255, 127, 80], # Coral (Corrected Name)
    'SideGarden': [255, 215, 0],    # Gold
...
ROOM_TYPES_TO_COUNT = [
    ...
    'OpenSpace', 'PrayerRoom', 'Staircase', 'Lobby', 'Lawn', 'Dining',
    'ServantQuarter', 'Passage', 'Laundry', 'DressingArea',
    'SideGarden', 'Library',
]
...
def analyze_floor_plan_updated(image_path):
    ...
    for accurate room counting and area calculation, including individual room areas.
    ...
    room_instances = defaultdict(list)  # New: Track individual room instances and their areas
    ...
    for room_name in ROOM_TYPES_TO_COUNT:
        ...
        count_for_room = 0
        area_for_room = 0
        instance_areas = []  # New: Store areas of individual instances
        ...
        for i in range(1, num_labels):
            ...
            if area >= MIN_AREA_THRESHOLD:
                count_for_room += 1
                area_for_room += area
                instance_areas.append(area)  # New: Add this instance's area
        ...
        for rn in room_names_for_color:
            if rn in ROOM_TYPES_TO_COUNT:  # Only update if it's a type we care about counting
                ...
                room_instances[rn] = instance_areas.copy()  # New: Store instances
    ...
    if expected_pixels and (width, height) != tuple(expected_pixels):
        logging.warning(
            f"Image dimensions ({width}x{height}) mismatch expected ({expected_pixels[0]}x{expected_pixels[1]}) for {file_name}")

    # Calculate actual room areas in square feet
    room_areas_sqft = {}
    room_instances_sqft = defaultdict(list)  # New: Store area of each instance in sq ft

    # Get total area in square feet
    total_area_sq_ft = dimensions.get('feet', [0, 0])[0] * dimensions.get('feet', [0, 0])[1]
    total_pixels = width * height

    # Conversion factor
    sq_ft_per_pixel = total_area_sq_ft / total_pixels if total_pixels > 0 else 0

    # Convert pixel counts to sq ft for each room type and instance
    for room_type, area_pixels in room_areas_pixels.items():
        room_areas_sqft[room_type] = round(area_pixels * sq_ft_per_pixel, 2)

    # Convert individual room instances to sq ft and sort from largest to smallest
    for room_type, instance_list in room_instances.items():
        sq_ft_areas = [round(pixels * sq_ft_per_pixel, 2) for pixels in instance_list]
        sq_ft_areas.sort(reverse=True)  # Sort from largest to smallest
        room_instances_sqft[room_type] = sq_ft_areas
    ...
    metadata = {
        ...
        # Add room areas in square feet
        'room_areas_sqft': {k: v for k, v in room_areas_sqft.items() if v > 0},
        # Add individual room instance areas
        'room_instances_sqft': dict(room_instances_sqft),
...
def save_extended_metadata_to_csv(all_metadata, output_filename):
    """
    Saves the extended metadata including individual room areas to a CSV file.

    Args:
        all_metadata (list): List of metadata dictionaries.
    ...
    try:
        ...
        for meta in all_metadata:
            flat_dict = {
                # Basic metadata
                'FilePath': meta.get('file_name', ''),
                'PlotSize': meta.get('plot_type', ''),
                'FloorLevel': meta.get('floor_level', ''),
                'PlanType': meta.get('plan_type', ''),
                'FP_Number': meta.get('fp_number', ''),
                'Version': meta.get('version', ''),
                'ImageWidth': meta.get('dimensions', {}).get('pixels', [0, 0])[0],
                'ImageHeight': meta.get('dimensions', {}).get('pixels', [0, 0])[1],
                'TotalPixels': meta.get('total_area_pixels', 0),
                'TotalAreaSqFt': meta.get('dimensions', {}).get('feet', [0, 0])[0] *
                                 meta.get('dimensions', {}).get('feet', [0, 0])[1]
            }

            # Add room counts
            for room_type, count in meta.get('room_counts', {}).items():
                flat_dict[f'Count_{room_type}'] = count

            # Add total room areas in square feet
            for room_type, area in meta.get('room_areas_sqft', {}).items():
                flat_dict[f'Total_{room_type}_SqFt'] = area

            # Add individual room instance areas
            for room_type, areas in meta.get('room_instances_sqft', {}).items():
                for i, area in enumerate(areas, 1):
                    flat_dict[f'{room_type}{i}_SqFt'] = area
        ...
        # Organize columns
        base_cols = ['FilePath', 'PlotSize', 'FloorLevel', 'PlanType', 'FP_Number', 'Version',
                     'ImageWidth', 'ImageHeight', 'TotalPixels', 'TotalAreaSqFt']
        count_cols = sorted([col for col in df.columns if col.startswith('Count_')])
        total_area_cols = sorted([col for col in df.columns if col.startswith('Total_')])
        instance_cols = [col for col in df.columns
                         if not col in base_cols
                         and not col in count_cols
                         and not col in total_area_cols]

        # Sort instance cols to group by room type and then by number
        instance_cols.sort()

        all_cols_ordered = base_cols + count_cols + total_area_cols + instance_cols
        ...
        # Final dataframe with ordered columns
        df = df[all_cols_ordered]

        # Save to CSV
        df.to_csv(output_filename, index=False, encoding='utf-8')
        logging.info(f"Extended metadata successfully saved to {output_filename}")

    except Exception as e:
        logging.error(f"Error saving extended metadata to CSV: {e}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description='Generate JSON metadata for floor plan dataset using connected components analysis.')
    ...
    parser.add_argument('--csv_output', default="floor_plan_metadata_extended.csv",
                        help='Path to save the consolidated CSV file with extended metadata')
    ...
    # Process the dataset and collect metadata
    logging.info(f"Starting dataset processing from: {args.dataset}")
    logging.info(f"Outputting JSON metadata to: {args.output}")
    ...
    # Find subdirectories (like '5_marla', '10_marla')
    plot_type_dirs = [d for d in os.listdir(args.dataset) if os.path.isdir(os.path.join(args.dataset, d))]

    if not plot_type_dirs:
        logging.warning(f"No subdirectories found in {args.dataset}. Expected directories like '5_marla', etc.")
        sys.exit(1)

    for plot_dir_name in plot_type_dirs:
        current_subdir = os.path.join(args.dataset, plot_dir_name)
        logging.info(f"--- Processing directory: {current_subdir} ---")

        # Create corresponding output directory structure
        plot_output_dir = os.path.join(args.output, plot_dir_name, 'metadata', 'json')
        try:
            os.makedirs(plot_output_dir, exist_ok=True)
        except OSError as e:
            logging.error(f"Could not create output directory {plot_output_dir}: {e}. Skipping this directory.")
        ...
        for png_file in png_files:
            ...
            try:
                ...
                if metadata:
                    ...
                    # Add to our collection for CSV generation
        ...
        logging.info(
            f"Finished directory {plot_dir_name}. Processed: {subdir_processed}, Skipped/Errors: {subdir_error}")
        processed_count += subdir_processed
        error_count += subdir_error
    ...
    # Save the consolidated extended metadata to CSV
    if all_metadata:
        save_extended_metadata_to_csv(all_metadata, args.csv_output)
